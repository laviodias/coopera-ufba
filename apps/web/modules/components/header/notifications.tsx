  ...
  return notifications && notifications.length > 0 ? (
    <>
      <ul className="space-y-1 max-h-80 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300">
        {notifications?.map((notification) => (
          <li key={notification.id} className={`flex flex-col gap-2 p-2 ${notification.read ? "" : "bg-gray-100 cursor-pointer hover:bg-gray-200"}`}>
            {notification.url ? (
              <Link
              ...
              <span className="text-sm" onClick={() => markAsRead(notification.id)}>{notification.message}</span>
            )}
